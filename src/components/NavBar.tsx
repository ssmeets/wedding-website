    ...
    return (
        <nav className="md-:py1 px-4 py-1 md:px-6" aria-label="Main">
            <div className="mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-black md:flex-row md:items-center">
                ...
                <button type="button" className="fixed right-4 top-4 mb-4 p-2 text-3xl grid justify-items-end text-black md:hidden" aria-expanded={open}
                ...
                >
                    <button
                        type="button"
                        className="fixed right-4 top-4 mb-4 p-2 text-3xl grid justify-items-end gap-8 text-black md:hidden"
                        aria-expanded={open}
                    ...
                    >
                        ...
                        <div className="grid justify-items-end gap-4">
                            {menu.data.navigation.map((item) => {
                                return (
                                    <a className="font-menu uppercase text-lg" href={"#" + item.link}>{item.label}</a>
                                );
                            })}
                        </div>
                    </button>
