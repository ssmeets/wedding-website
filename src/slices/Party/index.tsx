...
const submit: Locales = {
  en: "View on map", nl: "Bekijk op de kaart", pt: "Ver no mapa"
}
...
const Party = ({ slice, context }: PartyProps): JSX.Element => {
  ...
  return (
    ...
    >
      <Bounded>
        <h1 className="text-center font-curly text-9xl">Schedule</h1>
        <div className="border-[1px] border-black font-content  w-full">

          <div className="p-10">
            {slice.primary.events.map((item, index) => (

              <div className={clsx("grid grid-cols-4 gap-4", (index % 2 == 0) ? "opacity-1" : "opacity-0 hidden")} ref={refs[index]} key={index}>
                <div className="col-span-3 row-start-1">
                  <div className="grid grid-cols-3 gap-8">
                    <div className="col-span-1">
                      <PrismicNextImage field={item.image} width={150} />
                    </div>
                    <div className="col-span-2">
                      <h2 className="text-4xl pb-4">{item.event_title}</h2>
                ...
                <div className="row-start-1">
                  ...
                  <Button onClick={() => window.open("https://www.google.com/maps/place/" + item.map.latitude + "," + item.map.longitude, "_blank")} className="font-content bg-black py-2 px-4 text-sm text-white uppercase data-[hover]:bg-gray-600 data-[active]:bg-gray-700">
