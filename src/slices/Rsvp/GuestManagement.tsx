...
export default function GuestManagement({ context, guest, slice }: { context: string, guest: RSVPObject, slice: Content.RsvpSlice }) {
    ...
    const getDropDownMenu = (text: string) => {
        ...
        switch (text) {
            ...
            case 'dietary':
                return (guest.allergy == "allergy") && (<><RsvpTextArea key='dietary' setFunction={handleDietary} value={guest.allergytext || ""} placeholder={dietaryPlaceholder[context as 'en' | 'nl' | 'pt']} /><br /></>);
            case 'event':
                return (
                    <RsvpListBox key='event' setFunction={handlEvents} items={events} value={guest.events || []} context={context as 'en' | 'nl' | 'pt'} />
                );
            case 'song':
    ...
    return (
        <div>
            {parsedContent(slice.primary.attending)}<br /><br />
            {parsedContent(slice.primary.events_selection)}<br /><br />
            {parsedContent(slice.primary.food_selection)}<br /><br />
            {parsedContent(slice.primary.allergy_selection)}<br /><br />
            {parsedContent(slice.primary.song_suggestion)}<br /><br />
        </div>
    )
}
