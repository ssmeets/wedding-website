...
export function Guest({ context, slice, guest, index }: { context: string; slice: Content.RsvpSlice, guest: RSVPObject, index: number }) {
    ...
    <>
      <div className="block text-center font-content text-xl md:text-2xl leading-10 [overflow-anchor:none]">
        {rsvp.guests.map((guest, index) =>
          <Guest key={index} context={context} guest={guest} slice={slice} index={index} />
        )}
      </div>
    </>
  );
